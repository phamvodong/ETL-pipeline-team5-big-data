...
try:
    ...
    logger.info(f"Columns: {pd_df.columns}")
    logger.info(f"Sample row before conversion: {pd_df.iloc[0].to_dict() if not pd_df.empty else 'Empty DataFrame'}")
    ...
    for _, row in pd_df.iterrows():
        try:
            record = row.to_dict()
            # Ensure comments is a list, not numpy array or other type
            if 'comments' in record:
                # Convert comments to plain Python list if needed
    ...
    logger.info(f"Sample converted record: {records[0] if records else 'No records'}")
